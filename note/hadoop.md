...
zk -|-角色与特性   -|-选举 -|-超过半数投票才能成为Leader
    ...
    |               |            
    |               |           |-集群中唯一的Leader可执行写操作，执行写操作前进行投票（超过半数即可）
    |               |-Zab协议  -|
    |               |           |-集群中的Follower和Observer都可以执行读操作
----------------------------------------------------------------------------------------------------------------------------------
kafka集群：

        |-用LinkedIn开发的一个分布式消息系统
        |-是一个由Scala（Java）编写的消息中间件
        |       |-解耦、冗余、提高扩展性、缓冲
        |-功能 -|-保证顺序、灵活、削峰填谷
        |       |-异步通信
        |
...
kafka  -|-角色与结构   -|-topic：消息的类型                        -|-通过Zookeeper管理集群配置，选举Leader
        |               |-Parition：每个Topic包含一个或多个Parition-|
        |               |-Broker：kafka集群包括一个或多个服务器    -|
        |
        |           |-依赖于Zookeeper集群
        |           |-安装openjdk运行环境
        |-安装配置 -|-分发kafka拷贝到所有集群主机
        |           |-修改配置文件
        |           |-启动服务
--------------------------------------------------------------------------------------------------------------------------------
Hadoop高可用：
Namenode是集群中最重要的节点，存在单点故障风险


                            |-NN,ZK,ZKFailoverController   -|-把数据存放到共享存储，还需要考虑共享存储的高可用
            |-HDFS with NFS-|                                   |-都能实现热备份
            |               |-NFS                               |-都是一个active NN和一个standby NN
官方方案   -|                                              -|同-|
            |               |-NN,ZK,ZKFailoverController        |-都使用zk和zkfc来实现故障切换
            |-HDFS with QJM-|                                   |-失效切换都是用fencing配置的方法来active NN
                            |-JournalNode  -|-QJM不需要共享存储，但需要让每个DN都知道两个NN的位置，并把块信息和心跳包发给两个NN
